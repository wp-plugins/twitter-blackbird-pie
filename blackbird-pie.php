	...
	function shortcode($atts) {
		...
		if ($id) {
			...
			if ($http_code == "200") {
				...
				if ($profileBackgroundTile == "false")
				...
				if (!isset($dateFormat)) $dateFormat = __('F j, Y');
				$timeFormat = get_option('time_format');
				if (!isset($timeFormat)) $timeFormat = __('g:i a');
				$friendlyDate = date($dateFormat.' '.$timeFormat, strtotime($timeStamp));
